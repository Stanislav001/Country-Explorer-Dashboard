...
const UpdateHotelForm = () => {
    ...
    const updateHotelInitialValues: FormValues = {
        ...
        parkingIncluded: hotel?.hotel?.parkingIncluded || false,
        rating: hotel?.hotel?.rating || 1,
        price: hotel?.hotel?.price || 0,
        address: {
            StreetAddress: hotel?.hotel?.address.StreetAddress || '',
            City: hotel?.hotel?.address.City || '',
            StateProvince: hotel?.hotel?.address.StateProvince || '',
            PostalCode: hotel?.hotel?.address.PostalCode || '',
            Country: hotel?.hotel?.address.Country || '',
        },
        location: { coordinates: hotel?.hotel?.location.coordinates || [0, 0] }
    };

    const handleUpdateHotel = async (values: any) => {
        setIsSubmitting(true);
        try {
            const result = await hotelService.updateHotel(values, currentToken, hotel?.hotel?._id);
            ...
            if (result.status) {
                ...
                setErrorMessage(result.message || 'Failed to update hotel');
            }
        } catch (error) {
            setErrorMessage(error.message || 'Failed to update hotel');
        } finally {
    ...
    return (
        <Box sx={{ width: '100%' }}>
            <Formik
                initialValues={updateHotelInitialValues}
                    ...
                    <Form>
                        <Grid container spacing={3}>
                            ...
                            <Grid item xs={12}>
                                <LoadingButton
                                    fullWidth
                                    onClick={() => handleUpdateHotel(values)}
                                    size="large"
                                    color="inherit"
                                    variant="contained"
